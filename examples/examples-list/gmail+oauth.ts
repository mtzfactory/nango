...
export let syncGmailEmails = async (user: string, pizzlyProviderConfigKey: string, pizzlyConnectionId: string) => {
    let config = {
        ...
        pizzly_connection_id: pizzlyConnectionId,                  // Pre-configured Pizzly connection ID (cf. github.com/NangoHQ/Pizzly). TODO BB: change
        pizzly_provider_config_key: pizzlyProviderConfigKey        // Pre-configured Pizzly provider configuration (cf. github.com/NangoHQ/Pizzly). TODO BB: change
    };
