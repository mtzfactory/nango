...
If you haven't already, get a Slack access token [here](https://nangohq.notion.site/Quickstart-Slack-access-token-f41c7cc291c74fbd9b1110af6d631d01). This token is provided by us for the sake of this tutorial, but would otherwise belong to one of your users. With this token, we can tell Nango that a user has initiate a new connection with your Slack integration:
 ```typescript title="run-slack-notify.js"
await nango.registerConnection('slack', 1, '<slack-token-goes-here>').catch((e) => {console.log(e)});
```
